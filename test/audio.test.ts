...
test("effectiveVolume scales with master volume and per-cue loudness", () => {
	...
	const send = effectiveVolume(0.7, "send");
	assert.ok(complete > send, "complete (0.24) is louder than send (0.2)");
	assert.ok(send > 0 && send <= 1, "send volume is within (0, 1]");
});
...
test("effectiveVolume stays within (0, 1] and applies a floor near silence", () => {
	...
	const floor = effectiveVolume(0.001, "send");
...
test("resolveUisfxRoot locates the installed package and its scifi assets", () => {
	...
	assert.ok(existsSync(`${root}/sounds/scifi/send.mp3`), "scifi/send.mp3 exists");
});
